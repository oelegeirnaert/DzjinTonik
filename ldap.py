...
def test(config):
    ...
    #conn = Connection(server, user='{}\\{}'.format(domain_name, user_name), password=password, auto_bind=True)
    ...
    #conn.search("DC=medialaan,DC=vmma,DC=be", '(objectclass=person)', attributes=[ALL_ATTRIBUTES, ALL_OPERATIONAL_ATTRIBUTES])
    conn.search("DC=medialaan,DC=vmma,DC=be", search_filter = filter, search_scope=SUBTREE, attributes=[ALL_ATTRIBUTES, ALL_OPERATIONAL_ATTRIBUTES])
